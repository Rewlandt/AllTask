...
foreach($arResult as $arItem):?>
	...
	<?if ($arItem["IS_PARENT"]):?>
            ...
            <!-- [ex1-42] -->
                <?
                    ...
                    if (isset($arItem["PARAMS"]["CLASS_STYLE"])){
                        $sMenuColor = $arItem["PARAMS"]["CLASS_STYLE"];
                    }
                ?>
			<li>
                <a href="<?=$arItem["LINK"]?>" class="<?= $sMenuColor ?> <?if ($arItem["SELECTED"]):?>root-item-selected<?else:?>root-item<?endif?><?=$sClassStyle;?>">
                    <?=$arItem["TEXT"]?>
                </a>
			<ul>
            <!-- /[ex1-42] -->
		<?else:?>
			<li
                <?if ($arItem["SELECTED"]):?> class="<?= $sMenuColor ?> item-selected"<?endif?>>
